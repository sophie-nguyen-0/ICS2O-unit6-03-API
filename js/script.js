...
const checkWeather = async (URLAddress) => {
  try {
    ...
    document.getElementById('api-weather-icon').innerHTML = '<img src="' + 
      ...
      '" alt="API image" class="center" ' + ' width="20%" height="20%"'+
      '>' 
    document.getElementById('api-weather').innerHTML = '<h3>' + data.weather[0].description +'</h3>'+'<br>'+'<h4> temperature: '+(data.main.temp - 273.15).toFixed(2)+' feels like: '+(data.main.feels_like - 273.15).toFixed(2) +'</h4>'
  } catch (error) {
    console.log(error)
  }
}
